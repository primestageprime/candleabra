...
import { getClose, getHigh, getLow, getOpen } from "./utils.ts";
import {
  fiveMinuteish,
  hourish,
  LARGEST_GRANULARITY,
 ...
 */
export const updateAllTimeCandlestick =
  ...
  (accumulator: Accumulator): Accumulator => {
    ...
    const result: R.NonEmptyArray<Candlestick> = [
      toCandlestick([...accumulator.allSamples, ...samples]),
    ];
    return {
      ...accumulator,
...
export const makeProcessValue =
  ...
  (accumulator: Accumulator | null, value: number): Accumulator => {
    ...
    return updateAllTimeCandlestick(getLargestTierGranularity(sampleTiers))(
      updatedAccumulator,
    );
  };
