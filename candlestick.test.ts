...
const verifyLength = (length: number) =>
  R.pipe(
    R.prop("length"),
    R.equals(length),
  );

const verifyLatestAtomicSample = (expected: any) =>
  R.pipe(
    getLatestAtomicSample,
    R.equals(expected),
  );

const verifyLatestSamplesOf2 = (expected: any) =>
  R.pipe(
    getLatestSamplesOf2,
    R.equals(expected),
  );

const verifyLatestSamplesOf5 = (expected: any) =>
  R.pipe(
    getLatestSamplesOf5,
    R.equals(expected),
  );

const verifyAllSamples = (expected: any) =>
  R.pipe(
    R.prop("allSamples"),
    R.equals(expected),
  );
...
Deno.test("should process first value correctly", async (t) => {
  ...
  await t.step("verify samplesOf5 candlesticks", () => {
    assertEquals(
      verifyLength(1)(accumulator!.samplesOf5),
      true,
      "incorrect length",
    );
    assertEquals(
      verifyLatestSamplesOf5(nominal)(accumulator),
      true,
      "incorrect latest",
    );
  });
  ...
  await t.step("verify allSamples candlesticks", () => {
    assertEquals(
      verifyLength(1)(accumulator!.allSamples),
      true,
      "incorrect length",
    );
...
Deno.test("should process second value correctly", async (t) => {
  ...
  await t.step("verify two-sample candlesticks", () => {
    assertEquals(
      verifyLength(2)(accumulator.samplesOf2),
      true,
      `twoSamples: ${JSON.stringify(accumulator.twoSamples)}`,
    );
    assertEquals(
      verifyLatestSamplesOf2(nominal)(accumulator),
      true,
      `twoSamples: ${JSON.stringify(accumulator.twoSamples)}`,
    );
  });
...
Deno.test("should process 12th value correctly", async (t) => {
  ...
  await t.step("verify five-sample candlesticks", () => {
    assertEquals(verifyLength(1)(accumulator.samplesOf5), true);
    assertEquals(
      verifyLatestSamplesOf5(temporary_resolution)(accumulator),
      true,
    );
  });
